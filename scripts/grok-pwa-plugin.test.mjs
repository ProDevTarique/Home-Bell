...
test("names the install page from host slug", () => {
  assert.equal(appNameFromHost("localhost:8080"), "Home Bell");
  assert.equal(appNameFromHost("172.17.154.217:8080"), "Home Bell");
  assert.equal(appNameFromHost("wild-race.grok.me"), "Wild Race");
});

test("rejects hosts that are not plain slugs", () => {
  assert.equal(appNameFromHost("<script>alert(1)</script>"), "Home Bell");
  assert.equal(appNameFromHost('"><img src=x onerror=1>.grok.me'), "Home Bell");
});
...
test("renders the manifest with the per-app name", () => {
  ...
  assert.ok(manifest.icons.some((icon) => /icon-180|apple-touch-icon/.test(icon.src)));
});
